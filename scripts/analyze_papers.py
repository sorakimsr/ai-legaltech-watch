...
- **굵게 강조(`**...**`)는 "의미론적으로 의사결정에 직결되는 구절"에만 적용**.
  - 강조해야 할 것 (✅ 의미·영향력·시사점·행동 지침):
    예시) "단일 LLM 선택은 이미 낡았다", "책임 경계가 핵심 변수다", "RAG 파이프라인 재검토 시점이다",
    "성공한 목표당 비용으로 KPI를 재설계해야", "온프레미스 요구가 도입 속도를 제한", "에이전트 간 신뢰·조정 계층이 새 병목"
  - 강조 금지 (❌ 회사명·논문 제목·기법명·금액·기관명 같은 단순 고유명사):
    예시) "OpenAI", "Sequoia Capital", "Anthropic", "Foundation Protocol 논문",
    "AAIA-RAG-LEGAL", "Redrawing the AI Map", "수출입은행", "KB금융", "40억 달러", "Multi-Agent System"
  - 회사명·기법명·논문제목·금액은 본문에 그대로 적되 강조하지 말 것. 강조는 그 사실들이 "왜 의미 있는가/무엇을 시사하는가"의 해석 구절에만.
  - 한 문장에 1~2개 강조만. 과도한 강조 금지.

...
- `paper_indices`는 위 [논문 목록]의 실제 번호 (1부터 시작). 각 항목 당 2~6개 정도. 정확한 매칭이 어려운 인덱스는 넣지 말 것.
- JSON 외 텍스트 절대 금지
"""
...
def main():
    ...
    # paper_indices → 실제 논문 정보로 매핑 (hot_topics, key_techniques 둘 다)
    def attach_papers(items, ref_papers):
        out = []
        for it in (items or []):
            ...
            # paper_count가 LLM에 없거나 부정확하면 실제 매핑된 개수로 보정
            if "paper_count" not in new_item or not isinstance(new_item.get("paper_count"), int):
                new_item["paper_count"] = len(cited)
            out.append(new_item)
        return out

    sorted_for_ref = sorted(papers, key=lambda x: x.get("score", 0), reverse=True)[:MAX_PAPERS_FOR_LLM]

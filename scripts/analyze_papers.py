...
{{
  "narrative": "최근 흐름을 한국 전략·기획 담당자 관점에서 풀어쓴 분석. 1000~1500자, 4개 단락으로 구성된 자연스러운 평문. **마크다운 헤더(#, ##, ###) 사용 절대 금지**. \\n\\n**필수 구조 (이 형식을 정확히 따르세요)**:\\n\\n첫 단락: '한 줄 요약 (한 줄 요약 본문)'으로 시작. 한 문장으로 핵심 흐름을 압축.\\n\\n두 번째 단락: '1) 무엇이 부상하고 있는가 ' 로 시작. 구체 논문·주제 2~3개를 회사명·기법명 그대로 인용하면서 풀어 설명.\\n\\n세 번째 단락: '2) 한국 실무자에게 이 흐름이 무슨 의미인가 ' 로 시작. 전략·기획 담당자가 '아 그래서 어떻게 해야 하지'를 떠올릴 수 있도록 구체 사례로.\\n\\n네 번째 단락: '3) 산업 적용 흐름 ' 으로 시작. 어떤 분야(법률·금융·제조·콘텐츠 등)에 먼저 들어올 가능성이 높은가, 한국 기업이 채택 시 제약(개인정보·국외이전·on-prem 요구).\\n\\n각 단락은 반드시 빈 줄(\\\\n\\\\n)로 분리. 단락 번호 '1)', '2)', '3)' 표기 필수.",
  "hot_topics": [
  ...
  "actionable_insights": [
    "실무자가 본인 업무에 바로 시도해볼 수 있는 구체 시사점. 핵심 의사결정 구절은 `**...**`로 강조 (예: '**성공한 목표당 비용으로 KPI를 재설계해야** 한다'). 시점 표현(지금 당장 / 즉시 / 이번 주 / 이번 달 등) 금지.",
    "시사점 2 — 다른 관점/주제 (의미론적 강조 동일 정책)",
    "시사점 3 — 다른 관점/주제",
...
- **narrative에 마크다운 헤더(#, ##, ###) 절대 사용 금지**. 단락 구분은 빈 줄(`\\n\\n`)로만. 헤더로 섹션 나누지 말고 첫 문장이 단락 주제를 명확히 드러내도록 작성.
- 단락 사이는 반드시 빈 줄로 분리 (inline 으로 이어 쓰지 말 것).
- **굵게 강조(`**...**`)는 "의미론적으로 의사결정에 직결되는 구절"에만 적용**.
  ...
  강조 금지 (❌ 단순 고유명사·논문제목·기법명·금액·기관명):
    ...
    "Latent Cache Flow", "LFRAG", "BOHM", "Energy per Successful Goal"
  → 회사명·논문제목·기법명·금액·약어는 본문에 그대로 적되 **절대 `**...**` 마크다운 사용 X**.
  → 어떤 형태의 강조(굵게, 밑줄, 인용 등)도 회사명·기법명에는 사용 금지. 평문으로만 적기.
        ...
        "recent_papers": [
            {
                ...
                "url": p.get("url", ""),
                "source": p.get("source", ""),
                # v3.5: 발행일만 노출 (date_unknown=True면 빈 문자열 → UI에서 '—' 표시)
                "date": (p.get("date", "")[:10] if not p.get("date_unknown") else ""),

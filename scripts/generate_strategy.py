...
PROMPT_TEMPLATE = """당신은 **한국 대형로펌 경영전략팀의 시니어 컨설턴트**입니다. 독자는 전략·기획과 AI 업무를 동시에 수행하는 한국 실무자이며, **AI×법조 교차점이 가장 결정적인 관심사**입니다 (판결문 데이터 공개, 로펌 AI 도입, 변호사 AI 활용, 법무팀 AI 도입, 법조계 정책 등). 동시에 **AI 산업·기술의 프런티어 동향(플래그십 모델 release·빅테크 AI 제품·기술적으로 의미 있는 AI 연구·논문)을 그 자체로 충분히 파악하는 것도 전략팀의 핵심 임무**입니다 — 법조 함의로 환원되지 않더라도 '지금 AI가 어디까지 왔는가'를 아는 것 자체가 의사결정의 기반이기 때문입니다. **법조 교차 이슈를 가장 비중 있게 다루되, 프런티어 동향 인지를 잃지 않는 균형**을 원합니다. 모든 카드를 억지로 '법무 수요·리스크'로 환원하지 말 것.
...
v6.15.26 (2026-05-28): 카드 개수 정책 변경 — **{period_label} 기준 {card_count_rule}**.
의미 있는 trend가 있으면 모두 카드화, 억지로 채우지 말 것. 데이터에 등장한 사용자
...
- **굵게 강조(`**...**`)는 "의미론적으로 의사결정에 직결되는 구절"에만 적용** (사용자 정책):
  ...
  분량: 시사점 카드 1개(body+action)에 위 같은 의미 구절 **최소 3개, 최대 6개 강조 (필수, 위반 시 응답 거부 수준의 엄격한 룰)**. 한 문장 1~2개.
  ★ 동일 기준이 daily / weekly / monthly 모두 적용됨.
  ★★ 강조가 1개 이하인 카드는 응답 품질 기준 미달 — 반드시 body에서 인과·판단·시사 구절을
       찾아 추가로 `**...**` 처리하라. body에서 '~다는 점' / '~로 작동' / '~이 새 기준' 같은
       결론·판단 표현은 거의 항상 강조 대상이다. 회사명·금액·법명만 보고 끝내지 말 것.

- **카드 정렬 — 중요도 순 (v6.15.20)**: cards 배열의 **1번 카드가 가장 의사결정 영향력이 큰 것, 마지막 카드가 상대적으로 덜 중요한 것**.

  **★ 구성 균형 룰 (v6.15.51) — 법조 다수 유지 + 프런티어 동향 비중 확보 (대략 60:40)**:
  카드를 두 묶음으로 구성하되, 둘 다 충실히 채운다.
   (A) **법조·거버넌스 교차 카드 (다수, 약 60%)** — 아래 핵심 어젠다가 데이터에 잡혔다면 빠짐없이, 상위에 비중 있게 배치:
    · 판결문 공개·데이터 인프라 / AI 서비스 규제·개인정보(공정위·개인정보 처리 특례·AI 기본법) / 로펌·법무팀 AI 도입 / 법조계 AI 정책(대법원·법무부·변협) / AI 책임·소송·판례 / 글로벌 AI 거버넌스(EU AI Act·GPAI·미국 행정명령)
   (B) **프런티어 AI·기술 카드 (그 기간 데이터가 있으면 최소 4~5장 확보, 약 40%)** — 가장 중요한 다음을 **'그 발전 자체의 의미(역량 도약·방법론·시장 구조·경쟁 구도)'로** 다룬다. 법조와 직접 연결이 약해도 반드시 포함:
    · **플래그십 모델 release** (Anthropic·OpenAI·Google·Meta·xAI·Mistral 등 SOTA 모델·벤치마크 갱신 — 예: Claude Fable 5의 SWE-bench·장시간 자율 실행)
    · **빅테크 AI 제품·플랫폼·인프라** (Apple Intelligence/Siri 같은 OS·디바이스 AI 레이어, 코딩 에이전트, AI 반도체·데이터센터)
    · **기술적으로 의미 있는 AI 연구·논문** (새 방법론·추론·장문 생성·에이전트 구조 — 예: 'IS-CoT: Interleaved Structural Thinking'의 길이 붕괴(length collapse) 해결 같은 capability·method 진전). 산업 제품에 묻히지 않게 별도 카드로.
  → **(B) 카드는 법무 수요/리스크로 환원하지 말 것.** 본문은 그 발전 자체의 의미를 충실히 쓰고, 법조 함의가 있으면 **마지막 한 문장으로만** 덧붙인다(없으면 생략 가능). DeepSeek/Qwen/Cursor류 소식도 그 기간 가장 중요하면 (B)로 카드화.
  ...
  **★★ 코버리지 의무 (v6.15.26 — LLM 자율 판단)**:
  ...
  · **임팩트 크고 단독 의미 있는 이슈**: 기사 1건이어도 별도 카드 생성 권장.
     예: "공정위 AI 시장 실태조사 발표" 1건이지만 정책 출발점이라 단독 카드.
     예: "AI 개발 개인정보 처리 특례 입법 발의" 1건이지만 입법 흐름의 시그널.

  · **유사 범위·동일 주제**: 여러 기사여도 1개 카드로 묶기.
     예: 판결문 공개 학술대회 + 판결문 공개 찬반 설문 + 판결문 공개 범위 논의 3건
     → 1개 카드 ("판결문 공개·데이터 인프라" 주제)
     예: 김앤장 AI 도입 + 법무법인 광장 Tech&AI팀 + 세종 Harvey 도입 3건
     → 1개 카드 ("대형로펌 AI 도입 가속" 주제)

  · **같은 주제군 내 세부 이슈가 명확히 갈리면** 분리 가능:
     예: "판결문 공개 범위 확대"와 "판결문 비실명화 수준"이 별개 이슈로 충분히 다뤄지면
     → 2개 카드 가능

  품질 기준:
    · 6 주제군 중 데이터에 풍부히 등장한 주제가 카드에서 통째로 빠지면 미달.
    · 산업 trend(DeepSeek/Qwen/Cursor 등) 카드 갯수가 AI×법조 카드 갯수보다
      현저히 많으면 미달 (페르소나는 AI×법조 중심).
...
def generate_cards(items: list, period: str, ref_date: date, all_items: list):
    ...
    prompt = PROMPT_TEMPLATE.format(
        ...
        card_count_rule=card_count_rule,
    )
...
- **summary 안에서 가장 결정적인 1~2개 인사이트 구절을 `**...**`로 강조 필수**
  (예: "**거버넌스 설계 역량이 AI 도입의 실질적 병목**으로 부상"). 회사명·금액은 강조 X.
"""
    try:
...
# ============================================================================
# v6.15.21 (2026-05-28): 카드 중요도 재정렬 — 사용자 정책
#   "재생성하지 말고, 빌드 추가될 때마다 기존 시사점은 두고 새롭게 시사점을
#    생성해줘. 대신 중요도 순으로 재정렬하면 돼."
#
#   증분 빌드(_incremental) + 풀 빌드 양쪽에 적용. LLM 의존 없는 deterministic
#   ranking — AI×법조 교차 키워드 매칭 강도 기반 stable sort.
#   priority tier:
#     0 (top) : 사용자 핵심 AI×법조 어젠다 (판결문 공개·AI 규제·개인정보·로펌 AI)
#     1       : AI×법조 교차 일부 매칭 (high 키워드 1개만 매칭)
#     2       : 법조 일반 (법무·법원·변호사·소송 등 medium 키워드)
#     3       : 그 외 산업 일반 트렌드
#   같은 tier 내에서는 카드 원래 등장 순서 보존(stable sort).
# ============================================================================

# v6.15.20/.21 prompt 보호 어젠다와 동기화
_HIGH_PRIORITY_KEYWORDS = [
    # ① 판결문 공개·데이터 인프라
    "판결문", "비실명화", "익명화", "법인명 실명",
    "판결문 공개", "판결문 데이터", "판결문 데이터셋",
    # ② AI 서비스 규제·개인정보
    "공정위 ai", "공정거래위", "ai 시장 실태", "ai 서비스 시장",
    "ai 개인정보", "개인정보 처리 특례", "ai 개발 개인정보",
    "ai 기본법", "ai기본법", "인공지능 기본법",
    "정보통신망법", "개인정보보호법", "개인정보 보호법",
    # ③ 로펌·법무팀 AI 도입
    "로펌 ai", "변호사 ai", "법무팀 ai", "법조 ai", "ai 변호",
    "법률 ai", "리걸테크", "legaltech",
    # ④ 법조계 AI 정책
    "법원 ai", "법무부 ai", "대법원 ai", "헌법재판소 ai", "변협 ai",
    # ⑤ AI 책임·소송·판례
...
def _reorder_cards_by_importance(cards: list) -> list:
    ...
    indexed.sort(key=lambda x: (_card_priority(x[1]), x[0]))
    reordered = [dict(c) for _, c in indexed]
    # tag 'TREND XX · ...' 번호 재부여
    import re as _re_tag
    for i, c in enumerate(reordered, 1):
        tag = str(c.get("tag", ""))
        m = _re_tag.match(r"^\s*TREND\s+\d+\s*[·•]\s*(.+)$", tag, _re_tag.IGNORECASE)
        if m:
            c["tag"] = f"TREND {i:02d} · {m.group(1)}"
    return reordered
...
def main():
    ...
    # 기존: 빈 daily 시 prev news.json strategy(어제 카드)를 그대로 복제하던 로직.
    # 문제: KST 자정 직후 빌드에서 5/29 daily에 5/28 카드가 그대로 들어감.
    # 해결: 빈 daily는 history 저장 skip → 다음 빌드에서 자연 생성.
    ...
    if backend != "none":
        ...
        if not existing_daily_cards or force_refresh:
            ...
            # v6.15.21: 사용자 정책 — 증분 빌드 시 기존 + 신규 합쳐서 중요도 순 재정렬
            #   "재생성하지 말고, 빌드 추가될 때마다 기존 시사점은 두고 새롭게 시사점을
            #    생성해줘. 대신 중요도 순으로 재정렬하면 돼." (2026-05-28)
            #   재정렬 = AI×법조 교차 키워드 매칭 강도 기반 stable sort. tag 번호도 재부여.
            if existing_daily_cards:
                before_order = [c.get("tag", "")[:30] for c in existing_daily_cards[:5]]
                existing_daily_cards = _reorder_cards_by_importance(existing_daily_cards)
                after_order = [c.get("tag", "")[:30] for c in existing_daily_cards[:5]]
                if before_order != after_order:
                    print(f"  [daily 재정렬] 상위 5 tag 변동: {before_order} → {after_order}", flush=True)
                else:
                    print(f"  [daily 재정렬] 순서 변동 없음 (top 5)", flush=True)

    # v6.15.26 (2026-05-28): daily 카드 max 20 cap — 사용자 정책
    #   daily는 가독성 위해 최대 20개. weekly/monthly는 누적 시사점이라 cap 없음.
    #   재정렬 후 상위 20개만 유지 (중요도 낮은 카드부터 cut).
    DAILY_MAX_CARDS = 20
    if len(existing_daily_cards) > DAILY_MAX_CARDS:
        removed = len(existing_daily_cards) - DAILY_MAX_CARDS
        existing_daily_cards = existing_daily_cards[:DAILY_MAX_CARDS]
        print(f"  [daily cap] {len(existing_daily_cards) + removed} → {DAILY_MAX_CARDS} (중요도 낮은 {removed}개 cut)", flush=True)
        # tag 번호 재부여 (cap 후 1~20 연속 번호)
        import re as _re_cap
        for i, c in enumerate(existing_daily_cards, 1):
            tag = str(c.get("tag", ""))
            m = _re_cap.match(r"^\s*TREND\s+\d+\s*[·•]\s*(.+)$", tag, _re_cap.IGNORECASE)
            if m:
                c["tag"] = f"TREND {i:02d} · {m.group(1)}"

    # v6.15.28 (2026-05-29): fallback 5/28 카드 복제 버그 수정.
    #   기존: cards 0개 시 news.json의 prev strategy(전일 카드)를 그대로 복사 → 5/28 카드가
    #         5/29로 그대로 들어와 사용자가 "시사점 그대로" 보던 문제.
    #   변경: cards 0개 시 daily entry를 history에 저장하지 않음. 다음 빌드에서
    #         충분한 데이터로 자연 생성. fallback_strategy 사용 제거.
    #   효과: KST 자정 직후 빌드(5/29 publish 기사 적음) 시 5/28 카드 복제 방지.
    if not existing_daily_cards:
        print(f"  [daily] {today_iso} 카드 생성 데이터 부족 — history 저장 skip "
              f"(다음 빌드에서 재시도, fallback 복제 안 함)", flush=True)
    else:
